...
describe('ProductList pagination', () => {
  ...
  it('takes the page size from the parent', () => {
    const wrapper = mount(ProductList, { props: { products, perPage: 3 } })

    expect(names(wrapper)).toHaveLength(3)
    expect(wrapper.find(pageSelector).text()).toBe('Page 1 of 2')
  })

  it('drops the selection when the page changes', async () => {
    const wrapper = mount(ProductList, { props: { products, selected: products[0] } })
